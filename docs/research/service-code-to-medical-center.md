...
## 4.1 אימות מימוש (v1.2.x — רץ מול ה-DB בפועל)

- ETL מילא 3,713 ספקים ל-`provider_directory` (SyncRun); **מפתח ה-JOIN הוא `rehabSupplierId`** (5-6 ספרות), לא `modSupplierId` (SAP בן 10 ספרות) — אומת חי.
- כיסוי: **60%** מספקי ההסכם (326/547) קיימים בספרייה; **94%** (3,510/3,713) מהספקים עם מקצוע מזוהה.
- `GET /api/makt/:makt/institutions` נבדק על מק"טים אמיתיים (10769/10765/33507) → ~102 מרכזים כל אחד, 88 מועשרים.
- **פערי כיסוי ידועים (best-effort):** `imagingInstitutes` ו-`medicalEquipment` מחזירים 0 ברשימה הפשוטה — endpoint רשימת ההתמחויות שלהם מחזיר 404 וה-`specialty` מסנן (specialty=1→1093, specialty=2→13), כך שלא ניתן למנות אותם במלואם ללא רשימת ההתמחויות; הושארו כפער מתועד. 40% מספקי ההסכם שאינם בספרייה = ככל הנראה ספקי ציוד/לא-פעילים.
