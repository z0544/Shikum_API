...
const STOPWORDS = new Set(
  `אני אתה את אתם אנחנו הוא היא הם הן של על עם ב בב בבית גר גרה גרים
   ...
   נכה נכים נכות לדעת אופציה אופציות האופציות מגיע מגיעה מגיעות שמגיע שמגיעה שמגיעות
   זכאי זכאית זכאים זכאות אחוז אחוזי אחוזים פציעה פציעות נפגע נפגעתי סובל סובלת`
    .split(/\s+/)
    .filter(Boolean),
);
...
export class SearchService {
  ...
  private tokenize(text: string): string[] {
    ...
    for (const w of words) {
      ...
      // מספרים בודדים אינם מונחי-מוצר לחיפוש טקסט (מק"טים/קודים מטופלים בנפרד ב-searchByCode),
      // והם מייצרים התאמות-שווא מהכלה חלקית (למשל "80" בתוך "פרימייר 180").
  ...
  private async understand(
    ...
    const systemInstruction = [
      ...
      'שדה query = ביטוי חיפוש קצר וממוקד למוצר/שירות בלבד (למשל "טיפול פסיכולוגי", "כיסא גלגלים", "מכשיר שמיעה"). אל תכלול ב-query פרטים אישיים שאינם המוצר: אחוזי נכות, גיל, מספרים, סוג פציעה/מחלה, מצב רפואי או ניסוחי זכאות ("מה מגיע לי", "האופציות"). דוגמה: מתוך "אני נכה 80% עם פציעות גפיים ורוצה טיפול פסיכולוגי" → query="טיפול פסיכולוגי".',
